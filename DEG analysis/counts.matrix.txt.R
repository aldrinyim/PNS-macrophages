...
if (file.exists("counts.matrix.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("counts.matrix.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("../counts.matrix.txt", header=T, com='', row.names=1, check.names=F)
    primary_data = as.matrix(primary_data)
}
source("/Users/aldrinyim/utilities/trinityrnaseq-2.5.0/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/Users/aldrinyim/utilities/trinityrnaseq-2.5.0/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/Users/aldrinyim/utilities/trinityrnaseq-2.5.0/Analysis/DifferentialExpression/R/pairs3.R")
source("/Users/aldrinyim/utilities/trinityrnaseq-2.5.0/Analysis/DifferentialExpression/R/vioplot2.R")
data = primary_data
myheatcol = colorpanel(75, 'purple','black','yellow')
samples_data = read.table("../best_rep_samples.txt", header=F, check.names=F, fill=T)
...
write.table(data, file="counts.matrix.txt.minRow30.CPM.log2.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="counts.matrix.txt.minRow30.CPM.log2.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='ward.D2')
pdf("counts.matrix.txt.minRow30.CPM.log2.sample_cor_matrix.pdf")
...
", "counts.matrix.txt.minRow30.CPM.log2") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
gene_cor = NULL
