...
if (file.exists("counts.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("counts.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("counts.matrix", header=T, com='', sep="\t", row.names=1, check.names=F)
    primary_data = as.matrix(primary_data)
}
source("/Users/aldrinyim/utilities/trinityrnaseq-2.2.0/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/Users/aldrinyim/utilities/trinityrnaseq-2.2.0/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/Users/aldrinyim/utilities/trinityrnaseq-2.2.0/Analysis/DifferentialExpression/R/pairs3.R")
data = primary_data
...
write.table(data, file="counts.matrix.minRow10.CPM.log2.dat", quote=F, sep='	');
pdf("counts.matrix.minRow10.CPM.log2.principal_components.pdf")
data = as.matrix(data)
...
write.table(pca$rotation, file="counts.matrix.minRow10.CPM.log2.PCA.loadings", quote=F, sep="	")
write.table(pca$x, file="counts.matrix.minRow10.CPM.log2.PCA.scores", quote=F, sep="	")
