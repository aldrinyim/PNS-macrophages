...
parser.add_argument("-x", "--external", help= " ESSENTIAL External dataset with foldchange")
...
#external_dict structure is [gene name] = [fold-change,p-value]
external_dict = defaultdict(list)
ext_genes = []
einit = 0
with open(args.external) as f:
    for line in f:
        ...
        if einit == 0:
            ...
            external_dict[data[1]]=[float(data[2]),float(data[3])]
            ext_genes.append(data[1])
print(external_dict["Cybb"])
...
#annotated_genes = ("Sall1","Cx3cr1","Apoe","Ms4a7","Clec12a","Clec7a","P2ry12","Cybb","Ifi211","Axl")
annotated_genes = ("Clec12a","Cybb","H2-Aa","Aoah","P2ry12","Cp","Sall1","Apoe","Myo6","Ms4a7","Oasl2","Clec7a")
...
final_gene_list = list(set(gene_list) & set(ext_genes))
...
for g in final_gene_list:
    ...
    if pns > 0 and can > 0 and cns > 0:
        #print(g)
        pns_cns_ratio = np.log2(pns/cns)
        sall1_ratio = external_dict[g][0]
        #sall1_pvalue = external_dict[g][1]
        ...
        all_gene_coors_x.append(sall1_ratio)
        all_gene_coors_y.append(pns_cns_ratio)

        if pns_cns_ratio > 7.5:
            print(g,"| pns/cns ratio | ", pns_cns_ratio)
        #categorizing dots
    ...
    if g in annotated_genes:
        pns_cns_ratio = np.log2(pns/cns)
        sall1_ratio = external_dict[g][0]
        #sall1_p-value = external_dict[g][1]

        annotated_order.append(g)

        annotated_genes_coors_x.append(sall1_ratio)
        annotated_genes_coors_y.append(pns_cns_ratio)

        if g == 'Cx3cr1':
            print(g,'|',pns_cns_ratio,'|',sall1_ratio)
        if g == 'Sall1':
            print(g,'|',pns_cns_ratio,'|',sall1_ratio)
        if g == 'Trem2':
            print(g,'|',pns_cns_ratio,'|',sall1_ratio)
        ...
        re_gene_coors_x.append(sall1_ratio)
        re_gene_coors_y.append(pns_cns_ratio)
...
print('Stat : ',chi2,'|',chi2_red,'|',s_err)

x2 = np.linspace(np.min(x), np.max(x), 100)
y2 = np.linspace(np.min(y_model), np.max(y_model), 100)
'''

#slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(re_gene_coors_x, re_gene_coors_y)
#print('Slope : ',slope,'| intercept : ',intercept,'| r_value : ',r_value,'| p_value : ',p_value,'| std_err : ',std_err)

slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(all_gene_coors_x, all_gene_coors_y)
print('Slope : ',slope,'| intercept : ',intercept,'| r_value : ',r_value,'| p_value : ',p_value,'| std_err : ',std_err)
...
ax.set_xlim([min(all_gene_coors_x)-0.5,max(all_gene_coors_x)+0.5])
ax.set_ylim([min(all_gene_coors_y)-0.5,max(all_gene_coors_y)+0.5])
plt.scatter(annotated_genes_coors_x,annotated_genes_coors_y,s=50, alpha=0.9, c='red')
plt.scatter(re_gene_coors_x,re_gene_coors_y,s=dot_size, alpha=0.9, c='black')
...
for i in range(0,len(annotated_order)):
    ax.annotate(annotated_order[i],(annotated_genes_coors_x[i],annotated_genes_coors_y[i]),fontsize=15)
...
#plt.show()
plt.savefig('sall1-biplot_figs_prediction_interval_expressivity.pdf')
